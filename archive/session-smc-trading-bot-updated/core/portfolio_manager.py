...
_DEFAULT_CONFIG = {
    "portfolio": {
        "max_trades_per_day":    6,       # matches strategy_portfolio.yaml
...
class PortfolioManager:
    ...
    def evaluate(self, signals: list[Signal]) -> list[Signal]:
        ...
        # 5. Daily trade cap
        cap     = self._pcfg.get("max_trades_per_day", 6)
    ...
    def get_risk_pct(self, strategy_name: str) -> float:
        """Return risk % for this strategy.
        Reads from loaded config (strategy_portfolio.yaml) first;
        falls back to hardcoded tier table if not set.
        YAML 'risk' field stores percentage directly (e.g. 0.30 = 0.30%).
        """
        cfg_risk = self._strat_cfg.get(strategy_name, {}).get("risk")
        if cfg_risk is not None:
            return float(cfg_risk) / 100   # 0.30 → 0.003 fraction of account
        tier = _STRATEGY_TIER.get(strategy_name, "tier3")
        return RISK_TIERS[tier]

    def get_mode(self, strategy_name: str) -> str:
        """Return execution mode for this strategy: 'demo' | 'shadow'."""
        return self._strat_cfg.get(strategy_name, {}).get("mode", "shadow")
    ...
    def _strategy_allowed(self, signal: Signal) -> bool:
        ...
        if allowed_pairs and signal.symbol not in allowed_pairs:
            return False
        # Shadow-mode strategies must not enter the demo execution pipeline.
        # The signal_router / shadow_tracker handles them separately.
        if cfg.get("mode", "shadow") == "shadow":
            return False
        return True

    def is_shadow_strategy(self, strategy_name: str) -> bool:
        """True if this strategy is configured in shadow (observe-only) mode."""
        return self._strat_cfg.get(strategy_name, {}).get("mode", "shadow") == "shadow"
