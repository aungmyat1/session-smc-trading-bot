...
class ResearchEngine:
    ...
    def build_symbol(
        ...
        loaded = load_symbol_history(
            symbol, self.paths.raw_root, timeframe=timeframe, validate=False
        )
        report = validate_candles(
            loaded.frame, expected_freq=self._freq_for_timeframe(loaded.timeframe)
        )
        if not report.ok:
        ...
        smc_signals = self.signal_generator.generate(
            candles, sessions, structure, liquidity, fvg, order_blocks
        )
        breakout_signals = generate_london_breakout_signals(
            candles, pair=symbol, config=self.breakout_config
        )
        ny_signals = generate_ny_momentum_signals(
            candles, pair=symbol, config=self.ny_config
        )
        vwap_signals = generate_vwap_mean_reversion_signals(
            candles, pair=symbol, config=self.vwap_config
        )
        signals = pd.concat(
            [smc_signals, breakout_signals, ny_signals, vwap_signals],
            ignore_index=True,
            sort=False,
        )
        if not signals.empty:
            signals = signals.drop_duplicates(
                subset=["pair", "timestamp", "strategy_name", "direction"], keep="first"
            )
            signals = signals.sort_values(
                ["pair", "timestamp", "strategy_name"]
        ...
        self.dataset_store.write_outputs(
            outputs, symbol=symbol, timeframe=loaded.timeframe
        )
