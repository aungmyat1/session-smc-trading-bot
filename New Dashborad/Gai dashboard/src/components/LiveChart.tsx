      ...
      <div className="relative w-full overflow-hidden bg-zinc-950/80 border border-zinc-900 rounded-xl" style={{ height: `${height}px` }}>
        {candles.length === 0 ? (
          ...
          <svg className="w-full h-full select-none" viewBox={`0 0 ${width} ${height}`}>
            ...
            {/* Draw Asian Range Accumulation Box (0 to 12 candles representation) */}
            {(() => {
              const asianRange = pair.symbol === "EURUSD" 
                ? { high: 1.0858, low: 1.0820 } 
                : pair.symbol === "GBPUSD" 
                ? { high: 1.2775, low: 1.2735 } 
                : { high: 155.85, low: 155.25 };
              const topY = getY(asianRange.high);
              const botY = getY(asianRange.low);
              const boxHeight = botY - topY;
              const boxWidth = getX(Math.min(12, candles.length - 1)) - getX(0);
              ...
              return (
                <g>
                  ...
                  <rect
                    ...
                    width={boxWidth}
                    height={boxHeight}
                    className="fill-amber-500/[0.03] stroke-amber-500/25"
                    strokeWidth="1.2"
                    strokeDasharray="3 3"
                  />
                  ...
                  <text
                    x={getX(1)}
                    y={topY + 12}
                    className="fill-amber-400 font-mono text-[8px] font-bold tracking-wider"
                  >
                    ASIAN RANGE ({pair.symbol})
                  </text>
                  <text
                    x={chartWidth - 5}
                    y={topY - 4}
                    className="fill-amber-400/60 font-mono text-[7px] text-right font-bold"
                    textAnchor="end"
                  >
                    Asian High: {asianRange.high.toFixed(pair.symbol === "USDJPY" ? 2 : 5)}
                  </text>
                  <text
                    x={chartWidth - 5}
                    y={botY + 9}
                    ...
                    textAnchor="end"
                  >
                    Asian Low: {asianRange.low.toFixed(pair.symbol === "USDJPY" ? 2 : 5)}
                  </text>
                </g>
              );
            })()}

            {/* Draw Equilibrium (50% Fibonacci Retracement) & Premium vs Discount Zones */}
            {(() => {
              const eqLevel = (pair.swingHigh + pair.swingLow) / 2;
              const eqY = getY(eqLevel);
              return (
                <g>
                  {/* Equilibrium Line */}
                  <line
                  ...
                  <text
                    x="10"
                    y={eqY - 4}
                    className="fill-purple-400/80 font-mono text-[8px] font-bold"
                  >
                  ...
                  <text
                    x={chartWidth - 10}
                    y={22}
                    className="fill-rose-400/35 font-mono text-[8px] font-extrabold tracking-wider text-right"
                    textAnchor="end"
                  >
                    PREMIUM ZONE (SELL BIAS)
                  </text>
                  <text
                    x={chartWidth - 10}
                    y={height - 18}
                    className="fill-emerald-400/35 font-mono text-[8px] font-extrabold tracking-wider text-right"
                    textAnchor="end"
                  >
                    DISCOUNT ZONE (BUY BIAS)
                  </text>
                </g>
              );
            })()}
