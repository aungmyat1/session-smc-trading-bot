...
class StrategyVersion(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    strategy_id = Column(
        UUID(as_uuid=True), ForeignKey("strategy.strategy.id"), nullable=False
    )
...
class StageState(Base):
    ...
    strategy_id = Column(
        UUID(as_uuid=True),
        ForeignKey("strategy.strategy.id"),
        nullable=False,
        unique=True,
    )
...
class Approval(Base):
    ...
    gate_decision_id = Column(
        UUID(as_uuid=True), ForeignKey("governance.gate_decision.id")
    )
    from_stage = Column(String(50), nullable=False)
    to_stage = Column(String(50), nullable=False)
...
class StageTransition(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "to_revision = from_revision + 1", name="ck_stage_transition_revision"
        ),
        {"schema": "governance"},
    )
    ...
    version_id = Column(
        UUID(as_uuid=True), ForeignKey("strategy.version.id"), nullable=False
    )
    gate_decision_id = Column(
        UUID(as_uuid=True),
        ForeignKey("governance.gate_decision.id"),
        nullable=False,
        unique=True,
    )
    from_stage = Column(String(50), nullable=False)
...
class ArtifactBinding(Base):
    ...
    stage = Column(String(50), nullable=False)
    artifact_id = Column(
        UUID(as_uuid=True), ForeignKey("evidence.artifact.id"), nullable=False
    )
...
class ReportRecord(Base):
    ...
    json_artifact_id = Column(
        UUID(as_uuid=True), ForeignKey("evidence.artifact.id"), nullable=False
    )
    markdown_artifact_id = Column(
        UUID(as_uuid=True), ForeignKey("evidence.artifact.id")
    )
...
class ParameterSet(Base):
    ...
    experiment_id = Column(
        UUID(as_uuid=True), ForeignKey("experiments.experiment.id"), nullable=False
    )
...
class VirtualFill(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    order_id = Column(
        UUID(as_uuid=True), ForeignKey("execution.virtual_order.id"), nullable=False
    )
    symbol = Column(String(20), nullable=False)
    direction = Column(String(10), nullable=False)
    requested_price = Column(Numeric(12, 5))
