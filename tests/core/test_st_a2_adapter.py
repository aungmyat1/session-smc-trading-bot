...
def _make_raw_signal(
    side="long", entry=1.1000, sl=1.0950, tp=1.1100, risk_pips=50.0, session="london"
):
    raw = MagicMock()
    raw.side = side
    raw.entry = entry
...
def _make_candles(n=200):
    return [{"open": 1.1, "high": 1.11, "low": 1.09, "close": 1.105, "volume": 100}] * n
...
class TestST2Adapter:
    ...
    def test_returns_signal_on_valid_data(self, mock_gen):
        ...
        adapter = ST2Adapter()
        result = adapter.generate_signal(
            {"symbol": "EURUSD", "m15": _make_candles(), "h4": _make_candles(100)}
        )
    ...
    def test_none_when_strategy_import_unavailable(self):
        adapter = ST2Adapter()
        with patch.dict(
            "sys.modules", {"strategy.session_liquidity.session_strategy": None}
        ):
            result = adapter.generate_signal(
                {
                    "symbol": "EURUSD",
    ...
    def test_signal_sell_mapping(self):
        ...
        with patch(
            "strategies.adapters.st_a2_adapter.ST2Adapter.generate_signal",
            return_value=Signal(
                timestamp="t",
                strategy_name="ST-A2",
                symbol="GBPUSD",
