...
def build_structure(
    frame: pd.DataFrame, swings: pd.DataFrame, pair: str | None = None
) -> pd.DataFrame:
    ...
    _swing_lookup = (
        swing_df.set_index("timestamp") if not swing_df.empty else pd.DataFrame()
    )
    ...
    out = pd.DataFrame.from_records(
        records, columns=["timestamp", "pair", "structure", "direction"]
    )
    if not out.empty:
