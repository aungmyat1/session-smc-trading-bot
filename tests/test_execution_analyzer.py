...
def _fill(
    ts_str: str, sym: str = "EURUSD", entry_price: float = 1.0852, dry_run: bool = True
) -> dict:
    return _ev(
        "ORDER_FILLED",
        ts_str,
...
def _close(
    ts_str: str, sym: str = "EURUSD", result_r: float = 4.0, exit_reason: str = "tp2"
) -> dict:
    return _ev(
        "POSITION_CLOSED",
        ts_str,
...
def _rej(
    ts_str: str,
    sym: str = "EURUSD",
    reason: str = "SPREAD_TOO_WIDE:3.5pip",
    side: str = "buy",
) -> dict:
...
def _err(
    ts_str: str, sym: str = "EURUSD", msg: str = "timeout", ctx: str = "place_order"
) -> dict:
...
class TestFillToCloseDuration:
    def test_no_closed_trades_returns_status(self):
        ...
        result = compute_fill_to_close_duration(
            [_trade(close_ts=None, result_r=None, hold_minutes=None)]
        )
    ...
    def test_by_exit_reason_breakdown(self):
        ...
        assert result["by_exit_reason"]["tp2"]["avg_minutes"] == pytest.approx(
            95.0, abs=0.1
        )
...
class TestSpreadDistribution:
    ...
    def test_spread_too_wide_rejection_parsed(self):
        events = [
            ...
            _rej("2026-06-22 08:00:01", sym="EURUSD", reason="SPREAD_TOO_WIDE:3.5pip"),
        ]
        ...
        assert result["rejections_by_symbol"]["EURUSD"]["avg_pip"] == pytest.approx(
            3.5, abs=0.01
        )
    ...
    def test_rejection_rate_by_session(self):
        events = [
            _sig("2026-06-22 08:00:00", sym="EURUSD", sess="london"),
            _rej("2026-06-22 08:00:01", sym="EURUSD", reason="SPREAD_TOO_WIDE:3.0pip"),
...
class TestExecutionFailures:
    ...
    def test_fill_and_reject_rates_sum_to_one(self):
        ...
        assert result["fill_rate"] + result["reject_rate"] == pytest.approx(
            1.0, abs=0.001
        )
...
class TestDuplicateSignalAttempts:
    ...
    def test_same_entry_within_120s_flagged(self):
        ...
        sig1 = _sig(
            "2026-06-22 08:00:00", entry=1.0850, sym="EURUSD", sess="london", side="buy"
        )
        sig2 = _sig(
            "2026-06-22 08:00:00", entry=1.0850, sym="EURUSD", sess="london", side="buy"
        )
...
class TestRunIntegration:
    ...
    def _write_trades_jsonl(self, path: Path) -> None:
        ...
        records = [
            ...
            {
                "ts": "2026-06-22T08:05:01+00:00",
                "event": "ORDER_REJECTED",
                "symbol": "GBPUSD",
                "reason": "SPREAD_TOO_WIDE:4.2pip",
                "side": "sell",
            },
        ]
