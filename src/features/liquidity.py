...
def detect_liquidity_sweeps(frame: pd.DataFrame, pair: str | None = None) -> pd.DataFrame:
    ...
    return pd.DataFrame.from_records(records, columns=["timestamp", "pair", "sweep_type", "liquidity_level"])


def save_liquidity(frame: pd.DataFrame, path: str | Path, pair: str | None = None) -> Path:
