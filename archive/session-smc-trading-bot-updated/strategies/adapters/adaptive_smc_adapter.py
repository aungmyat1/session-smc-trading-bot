...
Fixes vs previous version:
  - smc_session_strategy exposes generate_signals() function, NOT a class.
    The old adapter imported a non-existent SMCSessionStrategy class and
    would silently return None on every call due to the ImportError catch.
  - risk_percent now reads 0.10 (shadow tier-3, per strategy_portfolio.yaml).
"""
...
class AdaptiveSMCAdapter(BaseStrategy):
    ...
    def generate_signal(self, data: dict) -> Optional[Signal]:
        ...
        raw_list = generate_signals(m15, h4, symbol)
