    ...
    def audit(self, context: AuditContext) -> AuditResult:
        ...
        if not grid:
            return AuditResult(
                ...
                "NOT_VERIFIED",
                0.0,
                recommendation="Provide parameter sweep results",
            )
        ...
        return AuditResult(
            self.name,
            "PASS" if stable else "PARTIAL",
            100.0 if stable else 55.0,
            metrics=grid,
            recommendation="Prefer stable parameter plateaus over fragile peaks",
        )
    ...
    def audit(self, context: AuditContext) -> AuditResult:
        if not context.parameter_grid:
            return AuditResult(
                ...
                "NOT_VERIFIED",
                0.0,
                recommendation="Provide sensitivity grid data",
            )
        return AuditResult(
            self.name,
            "PASS",
            100.0,
            metrics={"parameter_sets": len(context.parameter_grid)},
    ...
    def audit(self, context: AuditContext) -> AuditResult:
        ...
        if not stress:
            return AuditResult(
                ...
                "NOT_VERIFIED",
                0.0,
                recommendation="Provide stress-test outputs",
            )
        ...
        return AuditResult(
            ...
            metrics=stress,
            recommendation="Review spread/slippage/latency under stress",
        )
    ...
    def audit(self, context: AuditContext) -> AuditResult:
        ...
        if not scenarios:
            return AuditResult(
                ...
                "NOT_VERIFIED",
                0.0,
                recommendation="Provide scenario-testing evidence",
            )
        failures = [item for item in scenarios if not bool(item.get("passed", True))]
        return AuditResult(
            self.name,
            "PASS" if not failures else "PARTIAL",
            100.0 if not failures else 60.0,
            metrics={"scenario_count": len(scenarios), "failures": len(failures)},
            recommendation="Cover news, regime, and execution scenarios",
        )
