...
def compute_metrics(
    net_rs: list[float], exit_reasons: list[str] | None = None
) -> MetricSet:
...
def gate_check(trades: list[ReplayTrade]) -> GateResult:
    ...
    for name in strategy_names:
        ...
        if mode == "shadow":
            ...
            if not p_wr:
                notes_parts.append(
                    f"WR={m_std.win_pct()} < {GATE_MIN_WIN_RATE*100:.0f}%"
                )
...
def print_summary(trades: list[ReplayTrade], gate: GateResult) -> None:
    ...
    for g in gate.strategies:
        ...
        print(
            f"  {g.strategy:<20} {g.mode:<8} {m_std.n:>5} {pf_s:>8} {pf_2x:>8} "
    ...
    if demo_trades:
        ...
        for yr, m in sorted(yr_map.items()):
            ...
            print(
                f"  {yr:<8} {m.n:>5} {m.pf_str():>8} {m.win_pct():>7} "
    ...
    if demo_trades:
        ...
        for sess, m in sorted(sess_map.items()):
            print(
                f"  {sess:<12} {m.n:>5} {m.pf_str():>8} {m.win_pct():>7} {m.avg_r:>7.3f}"
            )
