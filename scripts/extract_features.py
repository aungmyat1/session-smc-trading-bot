...
def _load_csv_as_bars(sym: str, tf: str) -> list[dict]:
    ...
    csv_sym = (
        sym.replace("USD", "_USD")
        .replace("GBPUSD", "GBP_USD")
        .replace("EURUSD", "EUR_USD")
    )
...
def extract_sessions_and_sweeps(
    sym: str, start_dt=None, end_dt=None
) -> tuple[list, list]:
    ...
    log.info(
        "%s: running signal chain on %d M15 bars + %d H4 bars",
        sym,
        len(m15_bars),
        len(h4_bars),
    )
    ...
    for sig in signals:
        sweep_events.append(
            {
                "timestamp_utc": getattr(sig, "entry_time", None)
                or getattr(sig, "time", None),
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract SMC features from processed OHLCV"
    )
    parser.add_argument("--symbols", nargs="+", default=["EURUSD", "GBPUSD"])
    parser.add_argument(
        "--start", help="Start datetime prefix e.g. 2021-01-01T00:00:00Z"
    )
    ...
    for sym in args.symbols:
        ...
        session_events, sweep_events = extract_sessions_and_sweeps(
            sym, args.start, args.end
        )
        _write_feature(
            session_events, SESSION_COLS, DATA_FEAT / "sessions" / f"{sym}.parquet"
        )
        _write_feature(
            sweep_events, SWEEP_COLS, DATA_FEAT / "sweeps" / f"{sym}.parquet"
        )
