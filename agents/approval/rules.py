...
def evaluate_rules(
    reports: dict[str, Any], thresholds: dict[str, Any]
) -> list[RuleResult]:
    ...
    results.append(
        _score_rule(
            "SW-004",
            "Overall testing score ≥ threshold",
            mandatory=False,
    ...
    results.append(
        _score_rule(
            "TR-001",
            "Strategy validation score ≥ threshold",
            mandatory=True,
...
def _tool_detail(
    quality_report: dict[str, Any], stage: str, tool: str, key: str
) -> Any:
    s = quality_report.get(stage) or {}
    details = s.get("details", {})
