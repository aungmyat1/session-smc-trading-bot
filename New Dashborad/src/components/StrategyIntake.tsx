    ...
    <div className="bg-slate-50 border border-slate-200 rounded-lg p-6 mb-8 shadow-sm">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="font-display font-semibold text-slate-900 text-lg">AI-Assisted Strategy Intake Workstation</h2>
        </div>
        <button
          onClick={onClose}
          className="text-xs font-mono text-slate-400 hover:text-slate-600 px-2 py-1 border border-slate-200 hover:border-slate-300 rounded bg-white transition-colors"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 flex flex-col space-y-4">
          <div>
            ...
            <textarea
              ...
              className="w-full h-64 font-sans text-sm text-slate-900 bg-white border border-slate-200 rounded-md p-4 focus:ring-1 focus:ring-slate-900 focus:border-slate-900 outline-none resize-none transition-all"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {sampleIdeas.map((sample, idx) => (
                <button
                  ...
                  className="text-left border border-slate-200 hover:border-slate-300 bg-white p-3 rounded-md text-xs hover:bg-slate-50 transition-all group"
                  id={`load-sample-${idx}`}
                >
                  <span className="font-semibold text-slate-800 block mb-1 group-hover:text-slate-900">
                    {sample.title}
                  </span>
                  <span className="text-slate-500 line-clamp-2">{sample.text}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <button
            ...
            className="flex items-center justify-center space-x-2 w-full bg-slate-900 hover:bg-slate-800 text-white py-3 px-4 rounded-md text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <div className="lg:col-span-6 flex flex-col justify-between border border-slate-200 bg-white rounded-md p-5 min-h-[400px]">
          {isParsing && (
            <div className="flex flex-col justify-center items-center h-full py-16 space-y-4">
              <Cpu className="h-8 w-8 text-slate-600 animate-pulse" />
              ...
              <p className="text-xs font-mono text-slate-500 animate-pulse text-center max-w-xs px-4">
          ...
          {!isParsing && !parsedResult && !error && (
            <div className="flex flex-col justify-center items-center h-full py-16 text-slate-400">
          ...
          {error && (
            <div className="flex flex-col justify-center items-center h-full py-16 text-red-500 px-6 text-center">
            ...
            <div className="space-y-5 flex-1 overflow-y-auto max-h-[480px] pr-1">
              ...
              <div className="border-b border-slate-100 pb-3">
                <div className="flex justify-between items-start">
                  <div>
                    <span className="text-[10px] font-mono text-emerald-600 font-bold tracking-wider uppercase bg-emerald-50 border border-emerald-200 px-1.5 py-0.5 rounded">
                      Blueprint Compiled
                    </span>
                    <h3 className="font-display font-bold text-slate-900 text-lg mt-1.5">
                      {parsedResult.name}
                    </h3>
                  </div>
                  <div className="text-right">
                    <span className="text-[10px] font-mono text-slate-400 block uppercase">Completeness</span>
                    <span className={`text-base font-mono font-bold ${parsedResult.audit.score >= 80 ? "text-emerald-600" : "text-amber-600"}`}>
                      {parsedResult.audit.score}/100
                    </span>
                  </div>
                </div>
                <p className="text-xs text-slate-600 mt-2 italic">{parsedResult.description}</p>
              </div>

              {/* Extracted Rules */}
              <div className="grid grid-cols-3 gap-3 bg-slate-50 border border-slate-100 p-3 rounded-md font-mono text-[10px]">
                <div>
                  <span className="text-slate-400 block uppercase">Asset Class</span>
                  <span className="text-slate-800 font-bold">{parsedResult.rules.assetClass}</span>
                </div>
                <div>
                  <span className="text-slate-400 block uppercase">Symbol</span>
                  <span className="text-slate-800 font-bold">{parsedResult.rules.symbol}</span>
                </div>
                <div>
                  <span className="text-slate-400 block uppercase">Timeframe</span>
                  <span className="text-slate-800 font-bold">{parsedResult.rules.timeframe}</span>
                </div>
              </div>

              <div className="space-y-3">
                <div>
                  <h4 className="text-xs font-mono font-semibold text-slate-800 uppercase tracking-wider mb-1.5">
                    Entry Signals
                  </h4>
              ...
              <div className="border-t border-slate-100 pt-4">
                <h4 className="text-xs font-mono font-bold text-slate-800 uppercase tracking-wider mb-2 flex items-center space-x-1.5">
                  <Cpu className="h-3.5 w-3.5 text-indigo-600" />
                  <span>AI Logical Audit Results</span>
                </h4>

                {parsedResult.audit.logicalDefects.length === 0 ? (
                  <div className="bg-emerald-50 border border-emerald-100 rounded p-3 text-[11px] text-emerald-800 flex items-start space-x-2">
                    <CheckCircle className="h-4 w-4 text-emerald-600 shrink-0 mt-0.5" />
                    <div>
                      <p className="font-semibold">Zero logical defects detected.</p>
                      <p className="mt-0.5 text-emerald-700">Rules are fully specified and internally consistent.</p>
                    </div>
                  </div>
                ) : (
                      ...
                      >
                        <div className="flex items-start space-x-2">
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-slate-700 font-sans leading-relaxed">{defect.description}</p>
                            <p className="mt-1 text-[10px] text-slate-500 italic">Affected: "{defect.affectedRule}"</p>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              {parsedResult.audit.recommendations.length > 0 && (
                <div className="bg-slate-50 border border-slate-100 p-3 rounded text-[11px] text-slate-700 space-y-1">
                  <p className="font-bold font-mono text-[10px] text-slate-500 uppercase tracking-wider mb-1">Recommendations:</p>
              ...
              <div className="border-t border-slate-100 pt-4 flex justify-end space-x-2">
                <button
                  ...
                  className="px-3 py-1.5 border border-slate-200 hover:border-slate-300 text-slate-600 text-xs font-mono uppercase bg-white rounded-md transition-colors"
                  id="intake-reset-btn"
                >
                  Discard
                </button>
                <button
                  ...
                  className="px-4 py-1.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-mono uppercase rounded-md flex items-center space-x-1.5 transition-colors"
