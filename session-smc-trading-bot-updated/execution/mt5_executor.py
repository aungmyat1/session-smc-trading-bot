...
class MT5Executor:
    ...
    async def connect(self) -> None:
        try:
            ...
            raise RuntimeError("metaapi-cloud-sdk not installed — run: pip install metaapi-cloud-sdk>=29")
    ...
    async def get_symbol_price(self, symbol: str) -> dict:
        ...
        return {"bid": price.get("bid"), "ask": price.get("ask"), "time": price.get("time")}
    ...
    async def place_order(
        ...
        if direction == "long":
            result = await self._connection.create_market_buy_order(symbol, volume, sl, tp, opts)
        else:
            result = await self._connection.create_market_sell_order(symbol, volume, sl, tp, opts)

        order_id = result.get("orderId", result.get("id", "unknown"))
        entry = result.get("openPrice", 0.0)
        logger.info("Order placed: %s %s id=%s entry=%.5f", direction.upper(), symbol, order_id, entry)
    ...
    async def modify_position(self, position_id: str, sl: float, tp: float) -> bool:
        if not LIVE_TRADING:
            logger.info("[DRY RUN] Would modify position %s → sl=%.5f tp=%.5f", position_id, sl, tp)
            return True
