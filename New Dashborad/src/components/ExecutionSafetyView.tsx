...
export default function ExecutionSafetyView({ safetyReport }: ExecutionSafetyViewProps) {
  ...
  if (!safetyReport) {
    return (
      <div className="bg-white border border-slate-200 rounded-lg p-8 text-center text-slate-500">
        ...
        <p className="font-semibold text-sm">No Operational Safety Evidence</p>
        <p className="text-xs text-slate-500 mt-1">Please promote your strategy to the Execution Validation stage to audit infrastructure safety boundaries.</p>
      </div>
    );
  }
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "FAILED":
      default:
        return <AlertOctagon className="h-5 w-5 text-red-600 shrink-0 mt-0.5" />;
    }
  };
  ...
  const getStatusStyles = (status: string) => {
    switch (status) {
      case "PASSED":
        return "bg-emerald-50/50 border-emerald-100 text-slate-800";
      case "WARNING":
        return "bg-amber-50/50 border-amber-100 text-slate-800";
      case "FAILED":
      default:
        return "bg-red-50/50 border-red-100 text-slate-900";
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-8 bg-white border border-slate-200 rounded-lg p-5 shadow-sm space-y-4">
        <div>
          <h3 className="font-display font-semibold text-slate-900 text-sm flex items-center space-x-1.5">
            <HardDrive className="h-4.5 w-4.5 text-slate-600" />
          ...
          {safetyReport.safetyChecks.map((check, idx) => (
            ...
            >
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <h4 className="font-bold text-xs text-slate-900 leading-tight">{check.ruleName}</h4>
                  <p className="text-[11px] text-slate-600 mt-1 leading-relaxed">{check.description}</p>
                </div>
              </div>

              <div className="text-right font-mono text-[10px] shrink-0">
                <span className="text-slate-400 block uppercase">Observed</span>
                <span className="font-bold text-slate-900 block">{check.actualValue}</span>
                <span className="text-[9px] text-slate-400 block mt-1 uppercase">Threshold {check.thresholdValue}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="lg:col-span-4 space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm text-center">
          <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">Operational Safety Score</span>
          <span className="text-4xl font-mono font-bold block text-slate-900 tracking-tight mt-1.5">
            {safetyReport.signalIntegrityScore}/100
          </span>
          <div className="mt-3 inline-flex items-center space-x-1.5 px-2.5 py-1 text-[10px] font-mono font-bold bg-emerald-50 border border-emerald-200 text-emerald-800 rounded">
