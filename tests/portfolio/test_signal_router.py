...
def _sig(symbol="EURUSD", action="BUY", confidence=0.9,
         entry=1.10, sl=1.095, tp=1.11, ts=None, ttl=300) -> Signal:
    return Signal(
...
class TestDuplicateBlock:
    def test_same_symbol_direction_keeps_highest_confidence(self):
        router = SignalRouter()
        s1 = Signal(timestamp=_now(), strategy_name="ST-A2",
                    symbol="EURUSD", action="BUY",
                    entry_price=1.10, stop_loss=1.095, take_profit=1.11, confidence=0.9)
        s2 = Signal(timestamp=_now(), strategy_name="LondonBreakout",
                    symbol="EURUSD", action="BUY",
                    entry_price=1.10, stop_loss=1.095, take_profit=1.11, confidence=0.75)
    ...
    def test_sorted_by_confidence_desc(self):
        ...
        high = _sig(symbol="GBPUSD", action="BUY", confidence=0.95,
                    entry=1.27, sl=1.265, tp=1.28)
...
class TestGeometryValidation:
    ...
    def test_sell_sl_below_entry_rejected(self):
        router = SignalRouter()
        sig = Signal(timestamp=_now(), strategy_name="ST-A2",
                     symbol="EURUSD", action="SELL",
                     entry_price=1.10, stop_loss=1.09, take_profit=1.05,
                     confidence=0.9)
        assert router.route([sig]) == []
