...
def _add_issue(
    results: list[LayerIssue], severity: str, layer: str, message: str, **details: Any
) -> None:
    results.append(
        LayerIssue(severity=severity, layer=layer, message=message, details=details)
    )
...
def _check_parquet_schema(
    layer: str, path: Path, required_cols: set[str], issues: list[LayerIssue]
) -> dict[str, Any]:
    if not path.exists():
...
def _check_market_schema(path: Path, issues: list[LayerIssue]) -> dict[str, Any]:
    ...
    matched = any(
        ...
        for required_cols in REQUIRED_MARKET_SCHEMA_VARIANTS
    )
    if not matched:
        missing = [
...
def verify_layers(
    symbols: list[str], timeframes: list[str], max_age_hours: float
) -> dict[str, Any]:
    ...
    for layer, path in layer_dirs.items():
        ...
        results.append(
            LayerResult(
                layer=layer,
                status="PASS" if details.get("exists") else "FAIL",
                details=details,
            )
        )
    ...
    if not manifest:
        ...
        if not isinstance(layers, list) or not layers:
            _add_issue(
                ...
                "ERROR",
                "metadata",
                "layers manifest has no layer entries",
                path=str(manifest_path),
            )

    normalized_manifest_path = (
        DATA / "normalized" / "metadata" / "dataset_manifest.json"
    )
    ...
    if not normalized_manifest:
        _add_issue(
            ...
            "ERROR",
            "normalized",
            "missing normalized dataset manifest",
    ...
    for symbol in symbols:
        ...
        raw_tick_files = (
            sorted(raw_symbol_dir.rglob("ticks.parquet"))
            if raw_symbol_dir.exists()
            else []
        )
        if not raw_tick_files:
            _add_issue(
                issues, "ERROR", "raw", "no raw tick parquet files found", symbol=symbol
            )
        else:
            raw_path = raw_tick_files[0]
            raw_details = _check_parquet_schema(
                "raw", raw_path, REQUIRED_RAW_COLS, issues
            )
        ...
        norm_details = _check_parquet_schema(
            "normalized", norm_path, REQUIRED_NORMALIZED_COLS, issues
        )
        ...
        for tf in timeframes:
            ...
            market_parts = (
                sorted(market_root.glob("year=*/month=*/part-*.parquet"))
            ...
            proc_path = (
                market_parts[0]
                if market_parts
                else DATA / "processed" / symbol / f"{tf}.parquet"
            )
    ...
    if not feature_exists:
        ...
        if age is not None and age > max_age_hours:
            _add_issue(
                issues, "WARN", "analytics", "feature database is stale", age_hours=age
            )

    status = (
        "PASS" if not any(issue.severity == "ERROR" for issue in issues) else "FAIL"
    )
    return {
        "generated_at": _now(),
        "status": status,
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(
        ["", "## Summary", "", f"Result: **{report.get('status', 'UNKNOWN')}**"]
    )
    return "\n".join(lines) + "\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the layered research-data layout"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=72.0,
        help="Warn when files are older than this",
    )
