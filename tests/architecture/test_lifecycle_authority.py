...
def test_no_legacy_catalog_mutation_callers() -> None:
    ...
    for path in _active_python_files():
        relative = path.relative_to(ROOT)
        if (
            relative == Path("core/strategy_registry.py")
        ...
        for node in ast.walk(tree):
            if (
                ...
                and node.id in LEGACY_MUTATORS
            ):
                violations.append(f"{relative}:{node.lineno}: {node.id}")
            if (
                isinstance(node, ast.Attribute)
                and isinstance(node.ctx, ast.Load)
                and node.attr in LEGACY_MUTATORS
            ):
...
def test_lifecycle_code_does_not_write_catalog_projection() -> None:
    ...
    assert (
        not callers
    ), f"Compatibility projection writer used by active code: {callers}"
...
def test_catalog_has_no_active_or_approved_strategy() -> None:
    ...
    catalog = yaml.safe_load(
        (ROOT / "config/strategy_catalog.yaml").read_text(encoding="utf-8")
    )
    ...
    assert (
        not violations
    ), f"Strategies active or approved during construction: {violations}"
