...
def detect_swings(
    frame: pd.DataFrame, pair: str | None = None, lookback: int = 1
) -> pd.DataFrame:
    ...
    for i in range(lookback, len(df) - lookback):
        ...
        if (
            high == window_highs.max()
            and high > df.loc[i - 1, "high"]
            and high > df.loc[i + 1, "high"]
        ):
            records.append(
                {
    ...
    return pd.DataFrame.from_records(
        records, columns=["timestamp", "pair", "price", "swing_type"]
    )


def save_swings(
    frame: pd.DataFrame, path: str | Path, pair: str | None = None, lookback: int = 1
) -> Path:
