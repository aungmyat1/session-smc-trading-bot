...
from research_db.client import ResearchDB  # noqa: E402
...
def load_bars(sym: str, tf: str, start: str = None, end: str = None) -> list[dict]:
    ...
    if bars:
        log.info(
            "Loaded %d %s %s bars from CSV (Parquet not built)", len(bars), sym, tf
        )
    return bars
...
def write_baseline_report(
    ...
    for mo in sorted(monthly):
        ...
        lines.append(
            f"| {mo} | {m['n']} | {pct(m['win_rate'])} | {pfmt(m['pf'])} | {m['total_r']:+.2f}R |"
        )
...
def write_winner_loser_analysis(trades: list, run_id: str):
    winners = sorted(
        [t for t in trades if t["gross_r"] > 0], key=lambda t: -t["gross_r"]
    )
    losers = sorted(
        [t for t in trades if t["gross_r"] <= 0], key=lambda t: t["gross_r"]
    )
    ...
    def _table(ts):
        if not ts:
            return ["| — | No trades | — | — | — | — |"]
        rows = [
            "| Date | Session | Dir | SL pip | Gross R | Net R (std) | Exit |",
            "|---|---|---|---|---|---|---|",
        ]
        for t in ts[:20]:
            rows.append(
    ...
    w_lines = [f"# Winner Analysis | Run: {run_id}", "", "## Top Winners", ""] + _table(
        winners
    )
    l_lines = [f"# Loser Analysis | Run: {run_id}", "", "## All Losses", ""] + _table(
        losers
    )
...
def write_session_analysis(trades: list, run_id: str):
    ...
    lines = (
        [f"# Session Analysis | Run: {run_id}", ""]
        + _stats(london, "London")
        + [""]
        + _stats(ny, "New York")
    )
    ...
    for sess, ts in [("london", london), ("new_york", ny)]:
        for dirn in ["long", "short"]:
            ...
            if dt:
                ...
                lines.append(
                    f"| {sess} | {dirn} | {m['n']} | {pct(m['win_rate'])} | {m['avg_r']:+.3f} |"
                )
...
def run_replay(
    symbol: str, start: str, end: str, rr: float, dry_run: bool, db: ResearchDB
) -> str:
    ...
    log.info(
        "=== ST-A2 Replay | %s | %s → %s | RR=%.1f | run_id=%s ===",
        symbol,
        start,
        end,
        rr,
    ...
    ctx_start = (
        f"{int(start[:4]) - 1}-{start[5:]}" if start else None
    )  # 1yr prior for H4 context
    ...
    log.info(
        "Running signal chain on %d M15 + %d H4 bars...", len(m15_bars), len(h4_bars)
    )
    sigs, events = run_strategy(
        m15_bars, h4_bars, symbol, config={"rr": rr}, debug=True
    )

    # Filter to requested date range
    sigs_in_range = [
        s for s in sigs if start <= s.timestamp.strftime("%Y-%m-%d") <= end
    ]
    ...
    log.info(
        "n=%d WR=%s PF_std=%s PF_2x=%s MaxDD=%.2fR",
        m_std["n"],
        pct(m_std["win_rate"]),
        pfmt(m_std["pf"]),
        pfmt(m_stress["pf"]),
    ...
    if not dry_run:
        ...
        db.update_run_summary(
            run_id,
            {
                ...
                "pf_std": (
                    round(m_std["pf"], 4) if m_std["pf"] != float("inf") else 99.99
                ),
                "pf_2x": (
                    round(m_stress["pf"], 4)
                    if m_stress["pf"] != float("inf")
                    else 99.99
                ),
                "avg_r": round(m_std["avg_r"], 4),
                "total_r": round(m_std["total_r"], 4),
                "max_dd": round(m_std["max_dd"], 4),
                "verdict": verdict,
            },
        )
        ...
        db.save_strategy_metrics(
            {
                ...
                "session_london_pf": (
                    round(l_m["pf"], 4) if l_m["pf"] != float("inf") else 99.99
                ),
                "session_ny_pf": (
                    round(n_m["pf"], 4) if n_m["pf"] != float("inf") else 99.99
                ),
            }
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="ST-A2 replay with PostgreSQL research DB"
    )
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Run without writing to DB"
    )
    ...
    try:
        run_id = run_replay(
            args.symbol, args.start, args.end, args.rr, args.dry_run, db
        )
