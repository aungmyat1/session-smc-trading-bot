...
| 3 | Consolidate dashboard/API/strategy-loader duplication | **Backend done, frontend integrated and deployed (LIVE tab)** — 8 `/api/operations/*` endpoints (Phase 5) plus `/api/new-dashboard/live-state` (Phase 6) now on `status_server.py`, the only deployed backend; the Gai dashboard's LIVE tab consumes the latter end-to-end, browser-verified (2026-07-04). **Landed 2026-07-05**: the built SPA (`New Dashborad/Gai dashboard/dist/`, via `npx vite build`) is now actually served in production at `GET /new-dashboard/` (+ `/assets`) from `status_server.py` — previously this was only dev-server-verified, never deployed. `app.py`/`live_app.py` themselves not yet retired; SVOS/Suggestions tabs and Operator Controls (activate/pause/risk-controls/reconnect — their backend routes still don't exist) remain unwired |
