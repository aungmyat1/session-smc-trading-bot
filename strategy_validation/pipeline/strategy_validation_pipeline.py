...
class StrategyValidationPipeline:
    ...
    def run_document(self, document: StrategyDocument) -> ValidationReport:
        ...
        critical_issues = [finding.message for result in results for finding in result.findings if finding.severity == "ERROR"]
        warnings = [finding.message for result in results for finding in result.findings if finding.severity == "WARN"]
        audit_log = [
            AuditLogEntry(
