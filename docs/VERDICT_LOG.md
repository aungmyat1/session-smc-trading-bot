...
repointed to `--strategy ST-A2`. This correction does not delete or alter the original entry
above — it is preserved verbatim for traceability — and it does not mutate registry state or
lifecycle status, per the original entry's own scope note.
