...
CFG = {
    ...
    "sessions": {
        "asian":   {"start_h": 0,  "end_h": 8,  "range_thr": 0.50, "trend_thr": 0.70,
                    "first_close_pct": 0.75, "first_close_target": "opposite_box_edge", "trail_remainder": False},
        "london":  {"start_h": 7,  "end_h": 12, "range_thr": 0.55, "trend_thr": 0.75,
                    "first_close_pct": 0.75, "first_close_target": "opposite_box_edge", "trail_remainder": False},
        "overlap": {"start_h": 12, "end_h": 15, "range_thr": 0.60, "trend_thr": 0.80,
                    "first_close_pct": 0.75, "first_close_target": "4R", "trail_remainder": True},
        "newyork": {"start_h": 12, "end_h": 17, "range_thr": 0.55, "trend_thr": 0.75,
                    "first_close_pct": 0.75, "first_close_target": "opposite_box_edge", "trail_remainder": False},
    },
...
def _make_df_1h(hours: int = 48, base_price: float = 1.1000,
                amplitude: float = 0.0020) -> pd.DataFrame:
    """
    ...
    closes = base_price + amplitude * pd.Series(
        [0.5 * (i % 12) / 12 for i in range(hours)]
    ).values
    opens  = closes - 0.0001
    highs  = closes + 0.0003
    lows   = closes - 0.0003
...
class TestBuildSessionBox:
    ...
    def test_normal_asian_box(self):
        df = _make_df_1h(48)
        box = build_session_box(df, start_h=0, end_h=8,
                                instrument="EURUSD", session="asian")
    ...
    def test_too_few_candles_raises(self):
        ...
        with pytest.raises(ValueError, match="session not yet complete"):
            build_session_box(small_df, start_h=0, end_h=8,
                              instrument="EURUSD", session="asian")
...
class TestClassifySession:
    ...
    def _box(self, box_range, atr):
        return SessionBox(box_high=1.1050, box_low=1.1050 - box_range,
                          box_range=box_range, atr=atr, session="asian", instrument="EURUSD")
...
class TestDetectSweep:
    ...
    def _box(self):
        return SessionBox(box_high=1.1050, box_low=1.1000, box_range=0.0050,
                          atr=0.0030, session="asian", instrument="EURUSD")
...
class TestBuildSessionSignal:
    ...
    def test_eurusd_asian_low_sweep_long(self):
        ...
        with self._patch_structure("bullish"), self._patch_tp_engine(1.1200):
            with patch("smc_bot.session_range.detect_sweep") as mock_sweep, \
                 patch("smc_bot.session_range.build_session_box") as mock_box:

                mock_box.return_value = SessionBox(
                    box_high=1.1050, box_low=1.1000, box_range=0.0050,
                    atr=0.0030, session="asian", instrument="EURUSD",
                )
                sweep_candle = pd.Series({"open": 1.1010, "high": 1.1030,
                                          "low": 1.0992, "close": 1.1015})
                mock_sweep.return_value = SweepEvent(direction="low", candle=sweep_candle)
    ...
    def test_eurusd_asian_high_sweep_short(self):
        ...
        with self._patch_structure("bearish"), self._patch_tp_engine(1.0900):
            with patch("smc_bot.session_range.detect_sweep") as mock_sweep, \
                ...
                sweep_candle = pd.Series({"open": 1.1040, "high": 1.1060,
                                          "low": 1.1025, "close": 1.1038})
                mock_sweep.return_value = SweepEvent(direction="high", candle=sweep_candle)
    ...
    def test_xauusd_overlap_trend_long(self):
        ...
        with self._patch_structure("bullish"), \
             patch("smc_bot.session_range.tp_engine.build_plan",
                   return_value={"tp": 1920.0, "plan": []}):
            with patch("smc_bot.session_range.detect_sweep", return_value=None), \
                 patch("smc_bot.session_range.classify_session", return_value="trend"), \
                 patch("smc_bot.session_range.build_session_box") as mock_box:

                mock_box.return_value = SessionBox(
                    box_high=1910.0, box_low=1890.0, box_range=20.0,
                    atr=12.0, session="overlap", instrument="XAUUSD",
                )
    ...
    def test_gbpusd_excluded_from_asian_session(self):
        ...
        with self._patch_structure("bullish"):
            ...
            utc_now = datetime(2024, 1, 2, 9, 0, tzinfo=timezone.utc)  # after asian end_h=8
            ...
            with patch("smc_bot.session_range.build_session_signal", return_value=None) as mock_sig:
                ...
                assert len(asian_calls) == 0, "GBPUSD should be excluded from Asian session"
...
class TestScanAll:
    ...
    def _make_sig(self, instrument, session, weight=1.0):
        return SessionSignal(
            instrument=instrument, session=session, setup="sweep",
            side="long", entry=1.1020, sl=1.1000, tp=1.1120,
            box_high=1.1050, box_low=1.1000,
    ...
    def test_cap_at_max_concurrent_signals(self):
        ...
        with patch("smc_bot.session_range.build_session_signal", side_effect=signals[:4]):
