...
async def _tick(
    ...
    for symbol in _ALL_SYMBOLS:
        ...
        _log.info(
            "DATA %s  bars=%d  spread=%.1fpip  price=%.5f",
            symbol,
            len(m15),
            spread,
            px["bid"],
    ...
    if not raw_signals:
        _log.info(
            "No signals this tick (%d symbols scanned, %d strategies)",
            len(market),
            len(_STRATEGY_MAP),
        )
        return risk_state
    ...
    for signal in pm_approved:
        ...
        if not limit["approved"]:
            _log.info(
                "SKIP %s/%s — %s", signal.strategy_name, signal.symbol, limit["reason"]
            )
            continue
        ...
        try:
            ...
            _log.error(
                "Order failed %s/%s: %s", signal.strategy_name, signal.symbol, exc
            )
            _jdb.record_signal(
                signal,
                router_result="PASS",
...
async def run(mode: str, interval: int) -> None:
    _log.info(
        "Portfolio runner starting. MODE=%s  strategies=%s  symbols=%s",
        mode.upper(),
        list(_STRATEGY_MAP),
        _ALL_SYMBOLS,
    )
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=["shadow", "demo", "live"], default=env_mode)
