  ...
  if (!replayReport) {
    return (
      <div className="bg-white border border-slate-200 rounded-lg p-8 text-center text-slate-500">
        ...
        <p className="font-semibold text-sm">No Replay Evidence Generated</p>
        <p className="text-xs text-slate-500 mt-1">Please promote your strategy to the Historical Replay stage to run simulation engines.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm">
          <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">Total Simulated Return</span>
          <div className="flex items-baseline space-x-1.5 mt-1">
            <span className={`text-2xl font-mono font-bold ${replayReport.totalReturnPct >= 0 ? "text-emerald-600" : "text-red-600"}`}>
              {replayReport.totalReturnPct >= 0 ? "+" : ""}{totalReturn}%
            </span>
          </div>
          <span className="text-[10px] font-mono text-slate-500 block mt-1">Starting Capital: $100,000</span>
        </div>

        <div className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm">
          <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">Win Rate</span>
          <div className="flex items-baseline space-x-1.5 mt-1">
            <span className="text-2xl font-mono font-bold text-slate-900">{winRatePct}%</span>
          </div>
          <span className="text-[10px] font-mono text-slate-500 block mt-1">{replayReport.winningTrades} Wins / {replayReport.losingTrades} Losses</span>
        </div>

        <div className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm">
          <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">Profit Factor</span>
          <div className="flex items-baseline space-x-1.5 mt-1">
            <span className="text-2xl font-mono font-bold text-slate-900">{profitFactor}</span>
          </div>
          <span className="text-[10px] font-mono text-slate-500 block mt-1">Gross Win/Loss ratio</span>
        </div>

        <div className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm">
          <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">Maximum Drawdown</span>
          <div className="flex items-baseline space-x-1.5 mt-1">
            <span className="text-2xl font-mono font-bold text-red-600">{drawdownPct}%</span>
          </div>
          <span className="text-[10px] font-mono text-slate-500 block mt-1">Peak-to-trough drop</span>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-sans mt-0.5">Chronological trade compounding path</p>
          </div>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm">
        <div className="flex flex-col sm:flex-row justify-between sm:items-center border-b border-slate-100 pb-4 mb-4 gap-3">
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-sans mt-0.5">{replayReport.trades.length} trades recorded in history</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="font-sans text-xs bg-slate-50 border border-slate-200 rounded-md pl-8 pr-3 py-1.5 w-36 focus:outline-none focus:ring-1 focus:ring-slate-900"
                id="trades-search-input"
              />
            </div>

            {/* Filter buttons */}
            <div className="flex border border-slate-200 rounded-md p-0.5 bg-slate-50 text-[10px] font-mono">
              <button
                onClick={() => setTradeFilter("ALL")}
                className={`px-2 py-1 rounded ${tradeFilter === "ALL" ? "bg-white text-slate-900 font-bold shadow-sm" : "text-slate-500 hover:text-slate-800"}`}
              ...
              <button
                ...
                className={`px-2 py-1 rounded ${tradeFilter === "WIN" ? "bg-white text-emerald-600 font-bold shadow-sm" : "text-slate-500 hover:text-slate-800"}`}
              ...
              <button
                ...
                className={`px-2 py-1 rounded ${tradeFilter === "LOSS" ? "bg-white text-red-600 font-bold shadow-sm" : "text-slate-500 hover:text-slate-800"}`}
            ...
            <tbody className="divide-y divide-slate-50 font-mono text-[11px] text-slate-700">
              {filteredTrades.map((t) => (
                <tr key={t.id} className="hover:bg-slate-50">
                  ...
                  <td className="py-2.5">
                    <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${t.type === "BUY" ? "bg-emerald-50 text-emerald-700 border border-emerald-100" : "bg-red-50 text-red-700 border border-red-100"}`}>
                      {t.type}
                    </span>
                  </td>
                  <td className="py-2.5">
                    <span className="font-semibold">${t.entryPrice.toFixed(2)}</span>
                    <span className="block text-[9px] text-slate-400">{t.entryTime}</span>
                  </td>
                  <td className="py-2.5">
                    <span className="font-semibold">${t.exitPrice.toFixed(2)}</span>
                    <span className="block text-[9px] text-slate-400">{t.exitTime}</span>
                  </td>
                  <td className="py-2.5 text-right">{t.quantity.toFixed(0)}</td>
                  <td className={`py-2.5 text-right font-bold ${t.profitPct >= 0 ? "text-emerald-600" : "text-red-600"}`}>
                    {t.profitPct >= 0 ? "+" : ""}{t.profitPct.toFixed(2)}%
                  </td>
                  <td className={`py-2.5 text-right font-bold ${t.profit >= 0 ? "text-emerald-600" : "text-red-600"}`}>
              ...
              {filteredTrades.length === 0 && (
                <tr>
                  <td colSpan={7} className="text-center py-6 text-slate-400 italic">No historical trades found matching search filters.</td>
                </tr>
              )}
            </tbody>
          </table>
