...
def test_parameter_sweep_returns_summary(tmp_path):
    ...
    pd.DataFrame(
        {
            "timestamp": pd.date_range(
                "2024-01-01T08:00:00Z", periods=8, freq="15min", tz="UTC"
            ),
    ...
    summary = run_parameter_sweep(
        ...
        [
            ...
            SweepCandidate(
                name="strict", signal={"min_confluence": 2, "max_signals_per_day": 1}
            ),
        ],
    ...
    assert {"candidate", "n_trades", "avg_r", "expectancy", "profit_factor"}.issubset(
        summary.columns
    )
