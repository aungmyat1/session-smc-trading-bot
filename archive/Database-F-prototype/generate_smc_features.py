...
def detect_market_structure(df: pl.DataFrame, swings: pl.DataFrame) -> pl.DataFrame:
    ...
    if swings.is_empty():
        return pl.DataFrame(
            schema={"time": pl.Datetime, "structure": pl.Utf8, "t1": pl.Datetime}
        )
    ...
    for row in swings.iter_rows(named=True):
        if row["swing_type"] == "high":
            ...
            if prev_type == "high" and price > prev_price:
                structures.append(
                    {"time": row["time"], "structure": "HH", "t1": row["time"]}
                )
            elif prev_type == "low" and price > prev_price:
                structures.append(
                    {"time": row["time"], "structure": "BOS_BULL", "t1": row["time"]}
                )
            prev_type = "high"
            prev_price = price
        else:  # low
            ...
            if prev_type == "low" and price < prev_price:
                structures.append(
                    {"time": row["time"], "structure": "LL", "t1": row["time"]}
                )
            elif prev_type == "high" and price < prev_price:
                structures.append(
                    {"time": row["time"], "structure": "BOS_BEAR", "t1": row["time"]}
                )
            prev_type = "low"
            prev_price = price
...
def detect_liquidity_sweeps(df: pl.DataFrame, swings: pl.DataFrame) -> pl.DataFrame:
    ...
    if swings.is_empty():
        return pl.DataFrame(
            schema={"time": pl.Datetime, "sweep_type": pl.Utf8, "t1": pl.Datetime}
        )
...
def detect_order_blocks(df: pl.DataFrame, structure_df: pl.DataFrame) -> pl.DataFrame:
    ...
    bos_times = structure_df.filter(pl.col("structure").str.contains("BOS"))[
        "time"
    ].to_list()
...
def detect_fvg(df: pl.DataFrame) -> pl.DataFrame:
    ...
    if not fvgs:
        return pl.DataFrame(
            schema={"fvg_id": pl.Int64, "low": pl.Float64, "high": pl.Float64}
        )
