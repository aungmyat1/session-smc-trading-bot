...
def main():
    ...
    for sym in SYMBOLS:
        ...
        print(
            f'  {sym}: n={stats.get("n",0)}  gross_pf={stats.get("gross_pf")}  net_std={stats.get("net_pf_std")}  net_2x={stats.get("net_pf_2x")}  wr={stats.get("win_rate_pct")}%'
        )

    # Portfolio (combine both symbols)
    merged = (
        pd.concat(all_trades, ignore_index=True).sort_values("entry_time")
        if all_trades
        else pd.DataFrame()
    )
