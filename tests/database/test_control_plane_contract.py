...
def test_control_plane_models_include_revision_and_trust_guards() -> None:
    assert {"from_revision", "to_revision", "gate_decision_id"}.issubset(StageTransition.__table__.columns.keys())
    assert {"trust", "invalidated_at", "invalidation_reason"}.issubset(ArtifactBinding.__table__.columns.keys())
