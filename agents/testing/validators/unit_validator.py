    ...
    def _parse(
            ...
            try:
                ...
                for tc in suite.iter("testcase"):
                    ...
                    if failure is not None:
                        ...
                        errors.append(f"FAIL {tc.get('classname', '')}.{tc.get('name', '')}: {msg}")
        ...
        if cov_json.exists():
            try:
                ...
                coverage = round(float(raw.get("totals", {}).get("percent_covered", 0.0)), 1)
                details["coverage_pct"] = coverage
                if coverage < self._min_coverage:
                    errors.append(f"Coverage {coverage}% below required {self._min_coverage}%")
        ...
        failed = (n_failed > 0 or n_errors > 0) or (coverage is not None and coverage < self._min_coverage)
        return StageResult(
            name="unit_tests",
