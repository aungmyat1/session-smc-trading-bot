...
class TradeSimulator:
    ...
    def simulate(self, candles: pd.DataFrame, signals: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, sig in signals.iterrows():
            ...
            entry_pos = min(
                sig_pos + self.config.execution_delay_bars, len(candles) - 1
            )
            ...
            if is_long:
                stop_loss = float(
                    window["low"].min() - self.config.stop_buffer_pips * pip
                )
                entry_price = float(
                    entry_bar["open"]
                    + (self.config.spread_pips + self.config.slippage_pips) * pip
                )
            else:
                stop_loss = float(
                    window["high"].max() + self.config.stop_buffer_pips * pip
                )
                entry_price = float(
                    entry_bar["open"]
                    - (self.config.spread_pips + self.config.slippage_pips) * pip
            ...
            if risk <= 0:
                continue
            take_profit = (
                float(entry_price + self.config.rr_multiple * risk)
                if is_long
                else float(entry_price - self.config.rr_multiple * risk)
            )
            ...
            result_r = 0.0
            exit_limit = min(
                len(candles), entry_pos + max(1, self.config.max_hold_bars) + 1
            )
            ...
            rows.append(
                {
                    "trade_id": sha1(
                        f"{sig['signal_id']}-{sig_ts.isoformat()}".encode()
                    ).hexdigest()[:16],
