...
)  # Dec-Mar train, Apr-May validation
...
def load_symbol(symbol):
    ...
    df["median_spread"] = (
        df["spread"].rolling(24 * 4 * 10, min_periods=100).median().bfill()
    )
    return df
...
def summarize(trades: pd.DataFrame, initial=INITIAL_CAPITAL):
    ...
    return dict(
        ...
        profit_factor_R=(
            round(float(gross_win / gross_loss), 3) if gross_loss else math.inf
        ),
        max_drawdown_pct=round(float(dd.min() * 100), 2),
        final_equity=round(float(eq.iloc[-1]), 2),
        return_pct=round(float((eq.iloc[-1] / initial - 1) * 100), 2),
    )


def backtest(
    df: pd.DataFrame, symbol: str, p: Params, start=None, end=None
) -> pd.DataFrame:
    ...
    for i in range(200, len(dfx) - 1):
        ...
        if (
            np.isnan(pdh)
            or np.isnan(pdl)
            or np.isnan(phl_arr[i])
            or np.isnan(pll_arr[i])
        ):
            continue
...
def _make_trade_arr(
    ...
    if direction == "short":
        ...
        target = (
            liq
            if (
                p.target_mode == "liq_or_rr"
                and liq < entry
                and (entry - liq) / risk >= 1.2
            )
            else entry - p.rr * risk
        )
...
def make_trade(symbol, dfx, pending, p: Params, fill_i, entry, sl_buffer, pip):
    ...
    if direction == "short":
        ...
        if (
            p.target_mode == "liq_or_rr"
            and liq_target < entry
            and (entry - liq_target) / risk >= 1.2
        ):
            target = liq_target
        else:
            target = fixed_target
    ...
    return {
        "symbol": symbol,
        "entry_time": str(dfx.index[fill_i]),
        "direction": direction,
        "model": model,
        "entry_i": fill_i,
        "entry": float(entry),
...
def param_grid():
    ...
    for (ss, se), cb, mode, wait, rr, tgt, ms, tr in itertools.product(
        sessions, confirm, modes, waits, rrs, targets, maxstops, trends
    ):
...
def main():
    ...
    for n, p in enumerate(params, 1):
        ...
        if st["trades"] >= 20:  # avoid tiny sample winners
            ...
            results.append(
                {"params": p.__dict__, "score": score, "train": st, "validation": sv}
            )
        if n % 500 == 0:
    ...
    results = sorted(
        results,
        key=lambda x: (
            x["validation"]["profit_factor_R"],
            x["validation"]["total_R"],
            x["train"]["profit_factor_R"],
        ),
        reverse=True,
    )
    ...
    by_train = sorted(
        results,
        key=lambda x: (x["train"]["profit_factor_R"], x["train"]["total_R"]),
        reverse=True,
    )
    json.dump(
        {"best_by_validation": results[:20], "best_by_train": by_train[:20]},
        open(OUTDIR / "optimization_results.json", "w"),
        indent=2,
    )
    if not results:
        print(
            "No param sets reached 20 trades on train. Check data and sweep detection logic."
        )
        return
    best = results[0]
    ...
    for sym, df in data.items():
        ...
        per.append(st)
    merged = (
        pd.concat(all_tr, ignore_index=True).sort_values("entry_time")
        if all_tr
        else pd.DataFrame()
    )
    ...
    summary = {
        ...
        "full_period_per_symbol": per,
        "full_period_portfolio": port,
    }
    ...
    md = [
        ...
        "### Train",
        "```json",
        json.dumps(best["train"], indent=2),
        "```",
        "",
        "### Validation",
        "```json",
        json.dumps(best["validation"], indent=2),
        "```",
        "",
    ...
    for s in per:
        md.append(
            f"| {s['symbol']} | {s['trades']} | {s['win_rate_pct']}% | {s['total_R']} | {s['avg_R']} | {s['profit_factor_R']} | {s['max_drawdown_pct']}% | {s['return_pct']}% |"
        )
    md += [
        "",
        "### Portfolio proxy",
