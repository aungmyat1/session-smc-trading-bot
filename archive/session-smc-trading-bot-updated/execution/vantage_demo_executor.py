...
# VT Markets demo account uses -VIP suffix for forex symbols.
# Strategy and config always use canonical names (EURUSD, GBPUSD).
# This map translates at the broker API boundary only.
_BROKER_SYMBOL: dict[str, str] = {
    "EURUSD": "EURUSD-VIP",
    "GBPUSD": "GBPUSD-VIP",
    "USDJPY": "USDJPY-VIP",
    "XAUUSD": "XAUUSD-VIP",
...
def _bs(symbol: str) -> str:
    """Canonical symbol → broker-specific symbol name."""
    return _BROKER_SYMBOL.get(symbol, symbol)
...
class VantageDemoExecutor:
    ...
    async def get_price(self, symbol: str) -> dict:
        p = await self._rpc().get_symbol_price(_bs(symbol))
