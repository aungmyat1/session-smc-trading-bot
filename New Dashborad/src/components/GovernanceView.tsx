      ...
      <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm">
        ...
        <div className="overflow-x-auto dark-scrollbar max-h-96">
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-slate-50 font-mono text-[10px] text-slate-700">
              {strategy.auditLog.map((record) => (
                <tr key={record.id} className="hover:bg-slate-50">
                  <td className="py-3 font-bold text-slate-900">{record.id}</td>
                  <td className="py-3 text-slate-400">{new Date(record.timestamp).toLocaleString()}</td>
                  <td className="py-3 text-slate-800">{record.actor}</td>
                  <td className="py-3">
                  ...
                  <td className="py-3">
                    <div className="flex items-center space-x-1">
                      <span className="text-slate-400 truncate max-w-[80px]" title={record.fromStage}>{record.fromStage}</span>
                      {record.fromStage !== "NONE" && <ArrowRight className="h-3 w-3 text-slate-400 shrink-0" />}
                      <span className="font-bold text-slate-900 truncate max-w-[80px]" title={record.toStage}>{record.toStage}</span>
                    </div>
                  </td>
                  <td className="py-3 text-slate-500 text-[9px]" title={record.hash}>
                    {record.hash.slice(0, 10)}...
                  </td>
                  <td className="py-3 max-w-xs text-slate-600 font-sans leading-relaxed">
                    <p className="font-bold text-slate-800 font-mono text-[9px]">{record.evidenceSummary}</p>
