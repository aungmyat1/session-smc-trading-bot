...
from scripts.run_experiments import (apply_filter,  # noqa: E402
                                     compute_metrics, gate_check,
                                     run_all_experiments)
...
def _win(rr=5.0, sl_pips=20.0, sym="EURUSD", session="london"):
    return _trade(
        ...
        sl_pips=sl_pips,
        gross_r=rr,
        net_r_std=rr - 1.4 / sl_pips,
        net_r_2x=rr - 2.8 / sl_pips,
    )


def _timeout():
    return _trade(
        gross_r=1.5, net_r_std=1.43, net_r_2x=1.36, bars_held=96, exit_reason="timeout"
    )
...
class TestApplyFilter:
    ...
    def test_exclude_gbpusd_london(self):
        ...
        assert len(result) == 3
        assert not any(
            t["sym"] == "GBPUSD" and t["session"] == "london" for t in result
        )
...
class TestComputeMetrics:
    ...
    def test_max_dd_calculated(self):
        ...
        trades = [
            _trade(net_r_std=5.0),
            _trade(net_r_std=-1.0),
            _trade(net_r_std=-1.0),
            _trade(net_r_std=-1.0),
        ]
        m = compute_metrics(trades)
...
class TestRunAllExperiments:
    ...
    def test_exp03_ny_only(self):
        trades = self._make_trades()
        results = run_all_experiments(trades)
        r = next(r for r in results if r["exp_id"] == "EXP-03" and r["rr"] == 5.0)
    ...
    def test_exp04_excludes_gbp_london(self):
        trades = self._make_trades()
        results = run_all_experiments(trades)
        r = next(r for r in results if r["exp_id"] == "EXP-04" and r["rr"] == 5.0)
        # Count trades that are NOT (GBPUSD AND london)
        rr5 = [t for t in trades if t["rr"] == 5.0]
        expected = sum(
            1 for t in rr5 if not (t["sym"] == "GBPUSD" and t["session"] == "london")
        )
        assert r["n"] == expected
