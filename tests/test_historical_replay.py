...
from simulator.historical_replay import render_report, report_to_dict, run_historical_replay
...
def _asian_bars() -> list[dict]:
    prev = datetime(TRADE_DATE.year, TRADE_DATE.month, TRADE_DATE.day, tzinfo=_UTC) - timedelta(days=1)
...
def _full_day() -> list[dict]:
    bars = _asian_bars()
    bars.append(_bar(datetime(2024, 1, 15, 7, 0, tzinfo=_UTC), 1.0740, 1.0710, close=1.0730))
    bars.append(_bar(datetime(2024, 1, 15, 7, 15, tzinfo=_UTC), 1.0748, 1.0682, open_=1.0725, close=1.0720))
    bars.append(_bar(datetime(2024, 1, 15, 7, 30, tzinfo=_UTC), 1.0800, 1.0695, open_=1.0700, close=1.0790))
    return bars
