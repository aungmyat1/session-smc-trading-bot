...
// Real System 2 backend (dashboard/status_server.py, deployed as live-dashboard.service).
// Set BACKEND_URL to point elsewhere (e.g. an SSH-tunneled prod host) if not running locally.
...
async function startServer() {
  ...
  // Real-data passthrough — added for Phase 6 dashboard integration. Proxies to the
  // actual deployed backend instead of the simulated globalState below; no mock data,
  // no reimplemented business logic. This is the endpoint SocketContext.tsx polls.
  app.get("/api/new-dashboard/live-state", async (req, res) => {
    try {
      const upstream = await fetch(`${BACKEND_URL}/api/new-dashboard/live-state`);
      const body = await upstream.text();
      res.status(upstream.status).set("Content-Type", "application/json").send(body);
    } catch (err) {
      res.status(502).json({ error: "backend unreachable", detail: String(err), backend_url: BACKEND_URL });
    }
  });

  // API REST routes (simulated — kept for the SVOS/Suggestions tabs and as an offline
  // demo fallback; the LIVE tab now reads /api/new-dashboard/live-state above instead)
  ...
  // No WebSocket server: the real System 2 backend has none (WebSocket/Event Streaming
  // is a future milestone, not implemented yet — see SYSTEM2_MASTER_PLAN.md). Simulating
  // one here would show fake ticking data instead of the real backend's state, defeating
  // the point of this integration. Connection upgrades are rejected outright so the
  // frontend's existing WS-fail-then-poll fallback (SocketContext.tsx) engages immediately
  // against the real /api/new-dashboard/live-state proxy above, instead of a fake feed.
