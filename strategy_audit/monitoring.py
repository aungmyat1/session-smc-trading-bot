    ...
    def audit(self, context: AuditContext) -> AuditResult:
        ...
        if not historical or not live:
            return AuditResult(
                ...
                "NOT_VERIFIED",
                0.0,
                recommendation="Provide live and historical metrics",
            )
        warnings: list[str] = []
        drift = {}
        for key in (
        ...
        return AuditResult(
            ...
            warnings=warnings,
            metrics=drift,
            recommendation="Investigate production drift" if warnings else "Proceed",
        )
    ...
    def audit(self, context: AuditContext) -> AuditResult:
        if not context.notes.get("monitoring"):
            return AuditResult(
                ...
                "NOT_VERIFIED",
                0.0,
                recommendation="Provide monitoring evidence",
            )
        return AuditResult(
            self.name,
            "PASS",
            100.0,
            metrics=context.notes["monitoring"],
    ...
    def audit(self, context: AuditContext) -> AuditResult:
        ...
        if not alerts:
            return AuditResult(
                ...
                "NOT_VERIFIED",
                0.0,
                recommendation="Provide alerting configuration",
            )
        return AuditResult(
            self.name,
            "PASS",
    ...
    def audit(self, context: AuditContext) -> AuditResult:
        ...
        if not checks:
            return AuditResult(
                ...
                "NOT_VERIFIED",
                0.0,
                recommendation="Provide health-check results",
            )
        failed = [k for k, v in checks.items() if not bool(v)]
        return AuditResult(
            self.name,
            "PASS" if not failed else "PARTIAL",
            100.0 if not failed else 60.0,
            metrics=checks,
            recommendation="Fix failed health checks before deployment",
        )
