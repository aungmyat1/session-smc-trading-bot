...
def upgrade() -> None:
    ...
    op.add_column(
        "binding",
        sa.Column(
            "trust", sa.String(30), nullable=False, server_default="LEGACY_IMPORTED"
        ),
        schema="evidence",
    )
    op.add_column(
    ...
    op.add_column(
        "binding", sa.Column("invalidation_reason", sa.Text()), schema="evidence"
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "status IN ('PASS','FAIL','BLOCKED','IN_PROGRESS','NOT_RUN','INVALIDATED')",
            name="ck_report_status",
        ),
        sa.CheckConstraint(
            "trust IN ('QUALIFYING_REAL','SYNTHETIC','LEGACY_IMPORTED','INVALIDATED')",
            name="ck_report_trust",
        ),
        schema="evidence",
    )
    op.create_index(
        "idx_report_lookup",
        "report_record",
        ["strategy_id", "version_id", "stage", "created_at"],
        schema="evidence",
    )
