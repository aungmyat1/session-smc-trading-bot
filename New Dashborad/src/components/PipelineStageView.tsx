...
export default function PipelineStageView({ strategy, onPromote, onDemote }: PipelineStageViewProps) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 border-t border-slate-100 pt-5">
        {/* Gate Objectives */}
        <div className="md:col-span-8 flex flex-col justify-between">
          <div className="space-y-4">
            <div>
              <span className="text-[10px] font-mono text-slate-400 uppercase tracking-widest block">Active Phase</span>
              <h3 className="font-display font-bold text-slate-900 text-lg flex items-center space-x-2 mt-0.5">
                <span className="text-slate-400 font-mono">Stage {currentIdx + 1}:</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="bg-slate-50 border border-slate-100 p-3 rounded-md">
                ...
                <span className="text-xs font-semibold text-slate-800 flex items-center space-x-1.5 mt-1">
                  <Shield className="h-3.5 w-3.5 text-slate-600 animate-pulse" />
                  <span>
        ...
        {/* Action Controls */}
        <div className="md:col-span-4 flex flex-col justify-center border-l border-slate-100 pl-6 space-y-3">
          {currentIdx < stages.length - 1 ? (
            <button
              onClick={handlePromoteClick}
              disabled={isPromoting}
              className="flex items-center justify-center space-x-2 w-full bg-slate-900 hover:bg-slate-800 text-white py-3 px-4 rounded-md text-xs font-mono uppercase tracking-wider transition-colors disabled:opacity-50 shadow"
            ...
            <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-md p-3.5 text-center text-xs">
              <CheckCircle2 className="h-6 w-6 text-emerald-600 mx-auto mb-1.5" />
              <p className="font-bold">PRODUCTION APPROVED</p>
              <p className="text-[10px] text-emerald-700 mt-0.5">Strategy deployed on enterprise execution servers.</p>
            </div>
          )}
            ...
            <div className="pt-2">
              {!isDemoting ? (
                <button
                  ...
                  className="w-full text-center text-xs font-mono text-red-500 hover:text-red-700 py-1.5 border border-transparent hover:border-red-200 rounded transition-colors"
                  id="pipeline-show-demote-btn"
                >
                  [ Trigger Demotion / Revalidation ]
                </button>
              ) : (
                <div className="bg-red-50 border border-red-100 rounded-md p-3 space-y-2 text-left">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-mono text-red-600 font-bold uppercase">Demote Strategy</span>
                    <button
                  ...
                  <div>
                    <label className="block text-[9px] font-mono text-slate-500 uppercase mb-1">Target Stage</label>
                    <select
                      value={demoteTarget}
                      onChange={(e) => setDemoteTarget(e.target.value as ValidationStage)}
                      className="w-full bg-white border border-slate-200 rounded px-2 py-1 text-xs font-sans text-slate-900"
                  ...
                  <div>
                    <label className="block text-[9px] font-mono text-slate-500 uppercase mb-1">Audit/Risk Comments</label>
                    <input
                      type="text"
                      placeholder="E.g. Volatility slip threshold exceeded..."
                      value={demoteComments}
                      onChange={(e) => setDemoteComments(e.target.value)}
                      className="w-full bg-white border border-slate-200 rounded px-2 py-1 text-xs font-sans text-slate-900 outline-none"
                  ...
                  <button
                    ...
                    className="w-full bg-red-600 hover:bg-red-700 text-white py-1.5 rounded text-[10px] font-mono uppercase tracking-wider disabled:opacity-50"
