...
class RuleAuditModule(AuditModule):
    ...
    def audit(self, context: AuditContext) -> AuditResult:
        result = audit_strategy_text(
            context.strategy_text or context.notes.get("strategy_text", ""),
            strategy_name=context.strategy_name,
        )
        score = (
            100.0
            if result.status == "PASS"
            else 70.0 if result.status == "FIX" else 0.0
        )
        warnings = [
        ...
        return AuditResult(
            ...
            status=(
                "PASS"
                if result.status == "PASS"
                else "PARTIAL" if result.status == "FIX" else "FAIL"
            ),
            score=score,
            warnings=warnings,
            errors=errors,
            metrics={
                ...
                "inferred_fields": (
                    len(result.spec.inferred_fields) if result.spec else 0
                ),
            },
            recommendation=(
                "Refine the specification before testing"
                if errors or warnings
                else "Proceed to data audit"
            ),
    ...
    def audit(self, context: AuditContext) -> AuditResult:
        ...
        if not signals:
            return AuditResult(
                ...
                "NOT_VERIFIED",
                0.0,
                recommendation="Provide signal samples for signal validation",
            )
        missing = [
            s for s in signals if not isinstance(s, dict) or not s.get("signal_id")
        ]
        ...
        return AuditResult(
            ...
            warnings=[],
            errors=(
                [f"Missing signal_id in {len(missing)} signal samples"]
                if missing
                else []
            ),
            metrics={"signal_samples": len(signals), "invalid_signals": len(missing)},
            recommendation=(
                "Verify generated signals against expected entries"
                if missing
                else "Proceed to execution audit"
            ),
        )
    ...
    def audit(self, context: AuditContext) -> AuditResult:
        ...
        if not report:
            return AuditResult(
                ...
                "NOT_VERIFIED",
                0.0,
                recommendation="Provide a virtual demo execution report",
            )
        ...
        return AuditResult(
            ...
            status=(
                "PASS"
                if not errors and score >= 90
                else "PARTIAL" if score >= 70 else "FAIL"
            ),
            score=score,
            warnings=warnings,
            errors=errors,
            metrics={
                ...
                "execution_delay_ms_average": report.get(
                    "execution_delay_ms_average", 0.0
                ),
                "execution_delay_ms_maximum": report.get(
                    "execution_delay_ms_maximum", 0.0
                ),
            },
            recommendation=(
                "Fix execution errors before capital deployment"
                if errors
                else "Proceed to risk qualification"
            ),
            details=report,
        )
